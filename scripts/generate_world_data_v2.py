...
# ─── Flights ─────────────────────────────────────────────────────────────────
print("Generating flights (5-year range, 3-4+ per route per day)...")
...
for origin in iatas:
    for dest in iatas:
        ...
        if o_hub and d_hub:
            routes.append((origin, dest, 4, 6, 1.00))
        elif o_hub or d_hub:
            routes.append((origin, dest, 3, 4, 1.00))
        else:
            if random.random() < 0.55:
                routes.append((origin, dest, 2, 3, 0.72))
